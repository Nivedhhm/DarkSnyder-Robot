    ...
    if len(message.command) != 2:
        if message.from_user.id not in ADMINS: 
            buttons = [[
             ...
             InlineKeyboardButton("🗳 Deploy", url="https://t.me/DarkSnyder_rebot"),
             InlineKeyboardButton("🤖 Support", url="https://t.me/DarkSnyder_rebot")
             ]]
        else:
            buttons = [[
             InlineKeyboardButton("➕️ Add me to Your Chat ➕️", url=f"http://t.me/{bot_info.BOT_USERNAME}?startgroup=true")
...
async def help(bot, message):
    ...
    await message.reply_photo(
        ...
        caption=DarkSnyder.HELP_MSG.format(mention=message.from_user.mention),
        reply_markup=InlineKeyboardMarkup(button))
